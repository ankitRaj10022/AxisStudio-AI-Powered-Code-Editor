...
function highlightJavaScript(code: string): string {
  ...
  return code
    ...
    .replace(
      /(true|false|null|undefined|NaN)/g,
...
function highlightCSS(code: string): string {
  return code
    ...
    .replace(
      /([\-]?[0-9]+[\.]*[0-9]*(%|px|em|rem|vh|vw|ch|ex|pt|pc|cm|mm|in))/g,
      '<span style="color: #d19a66;">$1</span>',
    );
}
...
function highlightHTML(code: string): string {
  return code
    ...
    .replace(
      /(class|id|style|src|href|alt|title|width|height)=/g,
      '<span style="color: #d19a66;">$1=</span>',
    )
    .replace(/(".*?")/g, '<span style="color: #98c379;">$1</span>');
}
...
function highlightMarkdown(code: string): string {
  return code
    .replace(
      /^(# .+)$/gm,
      '<span style="color: #e06c75; font-size: 1.5em; font-weight: bold;">$1</span>',
    )
    .replace(
      /^(## .+)$/gm,
      '<span style="color: #e06c75; font-size: 1.3em; font-weight: bold;">$1</span>',
    )
    .replace(
      /^(### .+)$/gm,
      '<span style="color: #e06c75; font-size: 1.1em; font-weight: bold;">$1</span>',
    )
    ...
    .replace(
      /(`.*?`)/g,
      '<span style="background-color: rgba(0,0,0,0.1); padding: 0 3px; border-radius: 3px;">$1</span>',
    );
}
